...
class PidParserTest {
    ...
    @Test
    fun `parses unknown pid with generic formula`() {
        // 0xFF is unknown; getOrUnknown returns a generic definition that reads first 2 bytes.
        // Response only has one data byte (00) → 0.0
        val result = PidParser.parse(0xFF, "41 FF 00")
        assertNotNull(result)
